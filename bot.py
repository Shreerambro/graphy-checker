...
async def check_graphy_account(base_url, email, password):
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
        try:
            ...
            async with session.post(login_url, data=payload, headers=headers, allow_redirects=False, timeout=15) as resp:
...
async def graphy_cmd(client: Client, message: Message):
    ...
    try:
        combo_ask = await client.ask(chat_id, "📂 **Send Combo File (.txt) or Paste Combos (email:pass)**\n\n_Type /cancel to abort._", timeout=60)
